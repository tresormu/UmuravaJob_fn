...
import { fetchJobs, filterJobsForRecruiter, type JobRecord } from "@/services/jobsService";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadJobsData = async () => {
      ...
      try {
        ...
        const myJobs = filterJobsForRecruiter(allJobs, user.id);
        setJobs(myJobs);
  ...
  const handleUpdate = async () => {
    if (!accessToken || !user) return;
    try {
      const updatedUser = await updateRecruiter(accessToken, user.id, formData);
        ...
        <div className="relative p-8 md:p-12">
          <div className="flex flex-col md:flex-row items-center gap-8 w-full">
            ...
            <div className="flex-1 text-center md:text-left space-y-2">
              ...
              >
                <div className="flex items-center gap-2"><Briefcase className="w-4 h-4 text-accent" /> {user?.position || "Recruiter"}</div>
                <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-accent" /> {user?.companyName || "Umurava"}</div>
              </motion.div>
            </div>
      ...
      <div id="recruiter-details" className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  ...
                  <div className="flex items-center gap-4 group">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">{user?.position || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="p-3 bg-secondary rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">{user?.companyName || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="p-3 bg-secondary rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary truncate max-w-[150px]">{user?.companyWebsite || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="p-3 bg-secondary rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">{user?.phone || "Not specified"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="p-3 bg-secondary rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">{user?.timezone || "Africa/Kigali"}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 group">
                    <div className="p-3 bg-secondary rounded-xl text-primary group-hover:bg-primary group-hover:text-white transition-all">
                  ...
                  <div className="flex items-center gap-4 group">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-primary">January 2024</p>
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-12 pt-12 border-t border-border flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="flex items-center gap-4 group w-full md:w-auto">
                ...
                <div>
                  ...
                  <p className="font-bold text-primary uppercase text-xs tracking-tighter">
                    {user?.role === "recruiter" ? "Recruiter" : "System Administrator • Recruiter"}
                  </p>
                </div>
              </div>
        ...
        <div className="premium-card p-8">
          ...
          <div className="mt-8 space-y-4">
            {isLoadingJobs ? (
              <div className="p-12 text-center text-muted-foreground font-bold italic">Loading active postings...</div>
            ) : jobs.length > 0 ? (
              jobs.map((job, idx) => (
                <motion.div
                  key={job.id}
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-60">Active • {job.location || "Remote"}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="p-12 text-center text-muted-foreground font-bold bg-secondary/10 rounded-[2rem] border-2 border-dashed border-border">
