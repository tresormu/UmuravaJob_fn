...
export function OnboardingFlow() {
  ...
  return (
    <div className="h-screen w-full flex overflow-hidden font-sans">
      ...
      <div className="w-[45%] h-full bg-[#0055FF] relative flex flex-col justify-between p-12 text-white overflow-hidden">
        ...
        <div className="absolute bottom-[-20%] right-[-10%] w-full h-full bg-blue-400/20 rounded-full blur-[120px] pointer-events-none" />
        ...
        <div className="max-w-xl w-full space-y-12">
          ...
          <div className="flex gap-3">
            {SLIDES.map((_, i) => (
              <div 
                key={i} 
                className={`h-1.5 rounded-full transition-all duration-500 ${i === currentSlide ? "w-12 bg-blue-600" : "w-3 bg-blue-100"}`}
              />
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 pt-6">
            <button 
              onClick={handleNext}
              className="w-full sm:w-auto px-12 h-16 bg-blue-600 text-white font-black rounded-2xl shadow-2xl shadow-blue-200 hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 flex items-center justify-center gap-3 uppercase tracking-widest text-xs"
            >
            ...
            {currentSlide < SLIDES.length - 1 && (
              <button 
                ...
                className="text-slate-400 font-bold hover:text-blue-600 transition-colors text-xs uppercase tracking-widest"
              >
                Skip Onboarding
              </button>
            )}
